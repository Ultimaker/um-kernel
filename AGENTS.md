...
Welcome, AI Agent! This document defines the operational boundaries, design patterns, testing strategies, and collaborative conventions for the `um-kernel` repository. 
...
The **UltiMaker Linux Kernel** is a crucial customized Linux kernel built specifically for the printer's NXP ARM processor targets within the UltiMaker 3D printing ecosystem (specifically powering S-Line, Factor 4, and Factor 4+ series of professional/industrial 3D printers). 

### Business Value:
Optimizes low-level thread scheduling, limits memory latency, and provides customized drivers for active sensors and motion hardware interfaces.
...
### System Integration:
- `um-kernel` operates as a hardware system kernel on the printer's Linux environment.
- It interfaces with other components via Direct device tree configs (dts) and custom physical drivers.
...
gh skill install Ultimaker/UltiCortex jlink-embedded-dev
```
...
### Tech-Stack:
- **Languages**: C, Assembly, DTS
- **Build/Build Tooling**: GNU make & kbuild
- **Core Frameworks**: Custom Linux 5.x system bindings

### Core Directory Layout:
- `/configs/`: S-Line and Factor 4 defconfigs.
- `/dts/`: Device trees declarations.
...
1. Run compiler inside docker environment.
2. Execute build: `./build_for_ultimaker.sh`
