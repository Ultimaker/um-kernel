...
if mode == "-32":
    ...
    for i in range(0, len(pixels) - 1, pixelSize):
        pixels2 += bytes( [pixels[i + 2] ] )
        pixels2 += bytes( [pixels[i + 1] ] )
        pixels2 += bytes( [pixels[i] ] )
        pixels2 += bytes( [0] )
elif mode == "-24":
    ...
    for i in range(0, len(pixels) - 1, pixelSize):
        pixels2 += bytes( [ (pixels[i + 2] >> 3) | ( (pixels[i + 1] << 3) & 0xe0) ] )
        pixels2 += bytes( [ (pixels[i] & 0xf8) | ( (pixels[i + 1] >> 5) & 0x07) ] )
