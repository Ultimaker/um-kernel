...
parser.add_argument('-f' , '--format', dest='format'    , metavar='FORMAT', help="format,RGB,BGR,ARGB...",default='RGBA')
...
print(f'Image: {args.img_in} {w}X{h,im.mode}')
print(f'FrameBuffer:{args.buf_width}X{args.buf_height}{args.format}')

#if the size of image larger than than the framebuffer,cut it
if(w > args.buf_width):
	w = args.buf_width
	print(f'cut the Image width to {args.buf_width}')

if(h > args.buf_width):
	h = args.buf_height
	print(f'cut the Image height to {args.buf_width}')
